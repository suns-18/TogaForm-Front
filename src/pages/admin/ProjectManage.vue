...
const getList = async (page: number = 1) => {
    try {
        let res = await axios.post(`/project/queryList`, {
            ...
            projectName: keyword.value ? keyword.value : ""
        })
...
const showDetail = async (item) => {
    ...
    let createdByUser =
        await getUserById(item.createdBy)
    let lastUpdatedByUser =
        await getUserById(item.lastUpdatedBy)
            ...
            <v-card-text>
                <v-row class="ma-0 pa-0">
                    <v-col cols="2" class="text-center align-center mt-2">
                        <v-label class="text-center align-center">搜索项目：</v-label>
                    </v-col>
                ...
                <v-container v-if="viewList.data.length>0">
                    <v-expansion-panels flat>
                        <v-expansion-panel v-for="item in viewList.data"
                                           class="mt-2">
                            ...
                            <v-expansion-panel-text>
                                ...
                                <v-card v-if="item.quesnaires.length>0">
                                    <v-list>
                                        <v-list-item
                                                v-for="quesnaire in item.quesnaires">
                                            <v-row>
        ...
        <v-card>
            <v-container>
                ...
                <v-table density="compact">
                    ...
                    <tr>
                        <td>最近一次修改操作人</td>
                        <td v-if="lastUpdatedBy">{{ lastUpdatedBy }}</td>
                        <td v-else>
                            <v-progress-circular indeterminate></v-progress-circular>
                        </td>
                    </tr>
